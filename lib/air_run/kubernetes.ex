defmodule AirRun.Kubernetes do
  ...
  alias AirRun.Kubernetes.{KanikoBuildJob, Deployment, Service, Job}
  ...
  def make_poll_job(project_name, deployment_id, user_id) do
    job_config = Job.get_config(project_name, deployment_id, user_id)

    headers = [{"Content-type", "application/json"}]

    case post("/apis/batch/v1/namespaces/default/jobs", Poison.encode!(job_config), headers) do
      {:ok, %HTTPoison.Response{status_code: 201, body: body}} ->
        IO.puts("created")
        IO.inspect(body)
